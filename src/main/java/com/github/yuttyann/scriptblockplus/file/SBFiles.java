...
public final class SBFiles {
    ...
    public static void sendNotKeyMessages(@NotNull Plugin plugin, @NotNull YamlConfig yaml, @NotNull String path) {
        InputStream resource = FileUtils.getResource(plugin, path);
        if (resource == null) {
            return;
        }
        Set<String> keys = yaml.getKeys(true);
        YamlConfiguration config = YamlConfiguration.loadConfiguration(new InputStreamReader(resource, Charsets.UTF_8));
        String filePath = plugin.getName() + "/" + yaml.getFolderPath();
