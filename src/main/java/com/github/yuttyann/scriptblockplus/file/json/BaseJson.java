...
public abstract class BaseJson<T> {
    ...
    private File getJsonFile() {
        JsonTag jsonTag = getClass().getAnnotation(JsonTag.class);
        if (jsonTag == null) {
    ...
    public static String[] getNames(@NotNull Class<? extends BaseJson<?>> jsonClass) {
        ...
        File folder = new SBFile(ScriptBlock.getInstance().getDataFolder(), jsonTag.path());
        if (folder.exists()) {
