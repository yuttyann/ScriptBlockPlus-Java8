...
public final class LegacyReflectiveFactory implements TypeAdapterFactory {
    ...
    static {
        ...
        try {
            Class<?> runtime = Class.forName("com.google.gson.internal.bind.TypeAdapterRuntimeTypeWrapper");
        ...
        while (rawType != Object.class) {
            for (Field field : rawType.getDeclaredFields()) {
                ...
                for (int i = 0, l = fieldNames.size(); i < l; ++i) {
                    ...
                    BoundField replaced = result.put(name, createBoundField(gson, field, name, TypeToken.get(fieldType), serialize, deserialize));
                    if (previous == null) {
                        previous = replaced;
                    }
                }
